...
const endpoints = [
  ['POST', '/api/deploy', 'Deploy one HTML page. description is required. New short links include preserveHint and agentGuideUrl; appended versions stay quiet.'],
...
const workflows = [
  {
    name: 'New HTML app',
    steps: 'POST /api/deploy with description -> return url/detailUrl/versionUrl/preserveHint/agentGuideUrl -> tell the user to like it, read the guide, and reuse the same customCode for future versions',
  },
  {
...
export default function ApiDocsPage() {
  ...
  const text = isZh
    ? {
        ...
        intro: 'Agent 优先读取 OpenAPI；直接部署时必须使用 application/json，不要用 multipart/form-data。description 必填。新短链会返回点赞、持续迭代提示与 agentGuideUrl 使用指南；追加版本不会重复提示。',
    ...
    : {
        ...
        back: 'Back to Home',
        openapi: 'OpenAPI JSON',
        intro: 'Agents should read OpenAPI first. Deployments must use application/json, never multipart/form-data. description is required. New short links return preserve/iteration guidance and agentGuideUrl; appended versions do not repeat it.',
